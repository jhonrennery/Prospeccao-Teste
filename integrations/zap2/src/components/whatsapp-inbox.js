...
const DIRECT_CHAT_SERVER = '@s.whatsapp.net';

const normalizePhoneDigits = (value) => String(value ?? '').replace(/\D/g, '');

const buildDirectChatJid = (phoneDigits) =>
  phoneDigits ? `${phoneDigits}${DIRECT_CHAT_SERVER}` : null;

const readChatLaunchTarget = () => {
  ...
  return {
    phone,
    name: name || null,
    chatJid: buildDirectChatJid(phone),
  };
};

const chatMatchesLaunchTarget = (chat, target) => {
  if (!chat || !target?.phone) {
    return false;
  }
  ...
  return comparableValues.includes(target.phone);
};
...
export function WhatsAppInbox({ session }) {
  const initialChatLaunchTarget = readChatLaunchTarget();
  const [chatLaunchTarget, setChatLaunchTarget] = useState(() => initialChatLaunchTarget);
  ...
  const pendingLaunchChatJidRef = useRef(initialChatLaunchTarget?.chatJid ?? null);

  const loadChats = async () => {
    const response = await fetch(withAppBasePath('/api/inbox/chats'), {
      cache: 'no-store',
    });
  ...
  useEffect(() => {
    ...
    const resolveLaunchTarget = async () => {
      const existingChat = chats.find((chat) =>
        chatMatchesLaunchTarget(chat, chatLaunchTarget),
      );

      if (existingChat) {
        pendingLaunchChatJidRef.current = existingChat.chatJid;
        setShortcutError('');
        setSearch('');
        setActiveChatJid(existingChat.chatJid);
        setChatLaunchTarget(null);
        return;
      }

      try {
        const params = new URLSearchParams({
          phone: chatLaunchTarget.phone,
        });

        if (chatLaunchTarget.name) {
          params.set('name', chatLaunchTarget.name);
        }

        const response = await fetch(
          withAppBasePath(`/api/inbox/chats/resolve?${params.toString()}`),
          { cache: 'no-store' },
        );
        const data = await response.json().catch(() => null);

        if (!response.ok || !data?.chat?.chatJid) {
          throw new Error(
            data?.error ?? 'Nao foi possivel abrir a conversa deste contato.',
          );
        }
        ...
        pendingLaunchChatJidRef.current = data.chat.chatJid;
        setShortcutError('');
        setSearch('');
        setActiveChatJid(data.chat.chatJid);
      } catch (error) {
        if (cancelled) {
          return;
        }
        ...
        setShortcutError(
          error instanceof Error
            ? error.message
            : 'Nao foi possivel abrir a conversa deste contato.',
        );
      } finally {
        if (!cancelled) {
          setChatLaunchTarget(null);
  ...
  useEffect(() => {
    if (!activeChatJid && chats.length > 0) {
      const pendingChatJid = pendingLaunchChatJidRef.current;
      const fallbackChat =
        pendingChatJid && chats.some((chat) => chat.chatJid === pendingChatJid)
          ? pendingChatJid
          : chats[0].chatJid;

      setActiveChatJid(fallbackChat);
    }

    const launchTargetStillResolving =
      activeChatJid &&
      pendingLaunchChatJidRef.current === activeChatJid &&
      !chats.some((chat) => chat.chatJid === activeChatJid);

    if (launchTargetStillResolving) {
      return;
    }
  ...
  useEffect(() => {
    if (
      pendingLaunchChatJidRef.current &&
      activeChatJid === pendingLaunchChatJidRef.current &&
      chats.some((chat) => chat.chatJid === activeChatJid)
    ) {
      pendingLaunchChatJidRef.current = null;
    }
  }, [activeChatJid, chats]);
  ...
  useEffect(() => {
    const events = new EventSource(withAppBasePath('/api/inbox/events'));
  ...
  const sendAudioPayload = async (audio) => {
    ...
    const response = await fetch(withAppBasePath('/api/inbox/send'), {
      method: 'POST',
      body: formData,
    });
  ...
  return (
    <main className="wa-shell">
      <aside className="wa-sidebar">
        ...
        {shortcutError ? (
          <p className="wa-session-meta">{shortcutError}</p>
        ) : null}
